...
class PriorityReplayBuffer(ReplayBuffer):
    ...
    def __init__(self, dims=(14, 3, 1, 14, 1, 1), max_size=1000000):
        ...
        self.priority = np.zeros((int(max_size), 1))
...
class CasePool(object):
    ...
    def add(self, goal_, obstacle_):
        self.arg[self.cnt] = (goal_, obstacle_)
        self.score[self.cnt] = 10.0 * self.max_size
        self.cnt += 1
        if len(self.arg) > self.max_size:
            out = min([(-v, k) for k, v in self.score.items()])[1]
            del self.arg[out]
            del self.score[out]
    ...
    def result(self, score):
        ...
        for k in self.score:
            self.score[k] -= 10.0
        self.score[self.test_id] += score
        for k in self.score:
            self.score[k] = max(0.0, self.score[k])
        self.test_id = -1
    
    def sample(self):
        lst = list(self.arg.keys())
        self.test_id = lst[np.random.randint(len(lst))]
        return self.arg[self.test_id]
...
class NoObstacleAgent(object):
    ...
    def train(self, direct_start=False, total_step=None):
        ...
        try:
            ...
            obs, info, goal_, obstacle_ = self.env_reset(random_goal(), random_obstacle())
            # for global_step in range(args.total_timesteps):
            for global_step in range(0, total_step):
                # ALGO LOGIC: put action logic here
                if global_step < self.args.learning_starts and not direct_start:
                ...
                if not info.get("error", False):
                    # tj.append((obs, action, reward, next_obs, terminated))
                    rb.add((obs, action, reward, next_obs, terminated, -1000))
                ...
                if terminated or truncated:
                    ...
                    # pool = self.pool                    
                    # pool.result(total_reward)
                    # pool.add(goal_, obstacle_)
                    # goal_, obstacle_ = pool.sample()
                ...
                # ALGO LOGIC: training.
                if global_step > self.args.learning_starts or direct_start:
                    # batch_size = 4096
